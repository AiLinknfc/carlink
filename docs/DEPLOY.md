...
elegido en el nuevo wizard de 3 pasos para registrar Aceite (`ServiceFormModal.tsx` +
`frontend/src/lib/oilCatalog.ts`). Aditiva, no toca `lubricant_brand`/`lubricant_type` ni la
ficha pública de NFC (`NfcTokenInfoPublic` deliberadamente no incluye este campo). El entorno
automático bloqueó aplicarla desde acá (clasificada como "Production Deploy", correcto dado que
local/staging/prod comparten la misma base) — el usuario la corrió a mano. Verificado después:
`information_schema.columns` confirma la columna, y una lectura real de `maintenance_records`
(filas Aceite existentes) devuelve `lubricant_product = ''` en las filas viejas — backward
compatible, sin backfill necesario. `backend/app/models/models.py` y
`schemas.py::MaintenanceCreate/MaintenanceOut` ya la incluyen; `pytest` 70/70 y el backend local
(`uvicorn --reload`) verificados sanos antes y después del cambio de modelo (ver
docs/PENDIENTES.md sobre el susto de en medio: el modelo se había desplegado en el reload local
antes de que la migración existiera, y se revirtió temporalmente hasta que se aplicó).
