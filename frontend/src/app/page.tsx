    ...
    <div style={{ position: 'relative', minHeight: '100vh', height: '100vh', overflow: 'hidden', background: '#060606' }}>
      ...
      {authView && (
        <div style={{
          ...
          background: 'rgba(6,6,6,0.82)', backdropFilter: 'blur(18px)',
          animation: 'fadeUp .35s both',
        }}>
          <div style={{
            width: 'min(420px,94vw)', background: 'rgba(14,14,14,0.88)',
            backdropFilter: 'blur(28px)', border: '1px solid rgba(245,197,24,0.2)',
            borderRadius: 24, padding: '36px 32px 32px',
            boxShadow: '0 40px 100px rgba(0,0,0,.7)',
            textAlign: 'center',
          }}>
            <div style={{ fontSize: 11, letterSpacing: '.24em', textTransform: 'uppercase', fontWeight: 700, color: '#F5C518', marginBottom: 6 }}>
              Acceso seguro
            </div>

            <div style={{
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10,
              margin: '10px auto 16px', padding: '10px 18px',
              background: 'rgba(245,197,24,0.08)', border: '1px dashed rgba(245,197,24,0.3)',
              borderRadius: 12, width: 'fit-content',
            }}>
              <span style={{ fontFamily: "'Anton',sans-serif", fontSize: 22, color: '#F5C518', letterSpacing: '.05em' }}>{plateText}</span>
              <span style={{ width: 3, height: 3, borderRadius: '50%', background: '#7c786e' }} />
              <span style={{ fontSize: 13, color: '#b6b2a6' }}>{city}</span>
            </div>

            <p style={{ color: '#7c786e', fontSize: 13, margin: '0 0 22px', lineHeight: 1.5 }}>
              Elige tu cuenta de Google para continuar.
            </p>
            ...
            <button onClick={handleSignIn} disabled={loggingIn}
              style={{
                ...
                background: loggingIn ? 'rgba(255,255,255,0.05)' : '#fff',
                color: loggingIn ? '#7c786e' : '#202124',
                fontSize: 15, fontWeight: 600, cursor: loggingIn ? 'default' : 'pointer',
                transition: 'all .2s',
              }}
              onMouseEnter={e => { if (!loggingIn) e.currentTarget.style.background = '#f6f8fc' }}
              onMouseLeave={e => { if (!loggingIn) e.currentTarget.style.background = '#fff' }}>
              {loggingIn ? (
                <>
                  <div style={{ width: 20, height: 20, borderRadius: '50%', border: '2px solid rgba(255,255,255,0.2)', borderTopColor: '#F5C518', animation: 'spin .7s linear infinite' }} />
                  Verificando…
                </>
              ) : (
                <>
            ...
            <button onClick={() => setAuthView(false)}
              style={{ marginTop: 14, border: 'none', background: 'transparent', color: '#7c786e', fontSize: 13, cursor: 'pointer', padding: '6px 12px', borderRadius: 8 }}
              onMouseEnter={e => { e.currentTarget.style.color = '#b6b2a6' }}
              onMouseLeave={e => { e.currentTarget.style.color = '#7c786e' }}>
              Cancelar
            </button>
          </div>
        </div>
