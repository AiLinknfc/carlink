...
export default function DocumentosTab({ vehicleId, refreshKey }: Props) {
  ...
  const handleCreateDoc = useCallback(async () => {
    ...
    if (result) {
      flash(`${createName} creado — sube el archivo`)
      loadDocs()
  ...
  }, [vehicleId, createName, createType, createCode, createExpiry, flash, loadDocs])
  ...
  const saveEdit = useCallback(async (docId: string, type: string) => {
    const notesStr = [
      `status=vigente`,
    ...
    await apiPut(`/documents/${docId}`, { name: editName || undefined, notes: notesStr })
    flash('Documento actualizado')
    setEditingDoc(null)
    loadDocs()
  }, [editName, editExpiry, editCode, flash, loadDocs])
    ...
    <div style={{ animation: 'sectionIn .55s both', maxWidth: 880 }}>
          ...
          <div onClick={e => e.stopPropagation()} className="modal-panel" style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  <div style={{ fontFamily: "'Anton',sans-serif", fontSize: 18, textTransform: 'uppercase', lineHeight: 1.1, color: 'var(--text-1)' }}>Nuevo documento</div>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              ...
              <button onClick={handleCreateDoc} disabled={createSaving || !createName.trim()}
                ...
                {createSaving ? 'Creando…' : 'Crear documento'}
              </button>
            </div>
          </div>
        </div>,
      ...
      <div className="doc-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(260px,1fr))', gap: 14, animation: 'textIn .5s .1s both' }}>
          ...
          return (
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
                <div style={{ fontSize: 16, fontWeight: 700, minWidth: 0, wordBreak: 'break-word' }}>{dt.name}</div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, flex: '0 0 auto' }}>
                  {doc && (
                    <button onClick={() => openEdit(doc)} title="Editar documento" style={{
              ...
              {doc && (
                <>
                  {/* Preview */}
                  {doc.file_url && (
                    <div onClick={() => setLightboxUrl(doc.file_url)} style={{ position: 'relative', cursor: 'pointer', borderRadius: 11, overflow: 'hidden', marginTop: 14 }}>
                      {isPdfFile ? (
                        <div style={{
              ...
              {!doc && (
                <button onClick={() => openCreateModal(dt.type)}
                  style={{
              ...
              <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
                ...
                <div onClick={e => e.stopPropagation()} style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                    <h3 style={{ margin: 0, fontSize: 17, fontWeight: 800, color: 'var(--text-1)' }}>Editar documento</h3>
                  ...
                  <div style={{ display: 'flex', gap: 10 }}>
                    <button onClick={() => saveEdit(doc.id, dt.type)} style={{
        ...
        {documents.filter((d: any) => !DOCUMENT_TYPES.find(t => t.type === d.type)).map((doc: any) => {
          ...
          return (
            <Fragment key={doc.id}>
            <div style={{ padding: 18, borderRadius: 18, background: 'var(--surface)', border: `1px solid ${doc.file_url ? 'rgba(245,197,24,0.22)' : 'var(--border)'}` }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
                <div style={{ fontSize: 15, fontWeight: 700, minWidth: 0, wordBreak: 'break-word' }}>{doc.name}</div>
              ...
              <div style={{ marginTop: 12 }}>
                ...
                <div className="regGrid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6, marginTop: doc.file_url ? 10 : 0 }}>
                  <button onClick={() => handleDownload(doc)} style={{
                    display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4,
                    padding: '9px 4px', borderRadius: 10, border: '1px solid var(--border-2)',
                    background: 'var(--surface-2)', color: 'var(--text-2)',
                    fontSize: 10.5, fontWeight: 600, cursor: 'pointer', transition: 'all .18s', minHeight: 46,
                    opacity: doc.file_url ? 1 : 0.4,
                ...
                <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
                  ...
                  <div onClick={e => e.stopPropagation()} style={{
                    ...
                    <input type="date" value={editExpiry} onChange={e => setEditExpiry(e.target.value)}
                      style={{ width: '100%', padding: '10px 12px', borderRadius: 10, border: '1px solid var(--border-2)', background: 'var(--surface-2)', color: 'var(--text-1)', fontSize: 14, outline: 'none', marginBottom: 20, boxSizing: 'border-box' }} />
                    <div style={{ display: 'flex', gap: 10 }}>
                      <button onClick={() => saveEdit(doc.id, doc.type || 'custom')} style={{
                        flex: 1, padding: '11px 0', borderRadius: 10, border: 'none',
                        background: '#F5C518', color: '#111', fontSize: 13, fontWeight: 700,
                        cursor: 'pointer',
                      }}>Guardar</button>
