...
export default function QuickRegisterModal({ vehicleId, onClose, onSuccess, onSaved }: Props) {
  ...
  const [name, setName] = useState('Factura')
  ...
  const applyOcrResult = useCallback((result: OcrExtractResult, currentType: string) => {
    ...
    if (result.title && (!name.trim() || name === opt?.defaultName)) setName(result.title.slice(0, 60))
    if (result.vendor && !issuedBy.trim()) setIssuedBy(result.vendor)
    if (result.issue_date && (!issueDate || issueDate === today)) setIssueDate(result.issue_date)
    if (result.cost != null && !cost.trim()) setCost(String(result.cost))

    if (result.title || result.vendor || result.cost != null) {
      setOcrHint('Datos detectados automáticamente — revísalos antes de guardar')
    }
  }, [name, issuedBy, issueDate, cost])
  ...
  const processFile = useCallback(async (file: File, currentType: string) => {
    ...
    if (result) applyOcrResult(result, currentType)
    setScanning(false)
  }, [applyOcrResult])
      ...
      <div onClick={onClose} style={{
        ...
        <div onClick={e => e.stopPropagation()} style={{
          ...
          <div style={{ marginBottom: 18 }}>
            <label style={{ fontSize: 11, color: '#9a968a', fontWeight: 600, display: 'block', marginBottom: 5 }}>
              Foto o PDF del documento
            </label>

            {uploading ? (
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: 12, borderRadius: 10, background: 'rgba(245,197,24,0.08)', border: '1px solid rgba(245,197,24,0.3)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: 12, borderRadius: 10, background: 'rgba(46,204,113,0.06)', border: '1px solid rgba(46,204,113,0.25)' }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <label
                  ...
                  onMouseEnter={e => { e.currentTarget.style.background = 'rgba(245,197,24,0.1)'; e.currentTarget.style.borderColor = '#F5C518' }}
                  onMouseLeave={e => { e.currentTarget.style.background = 'rgba(245,197,24,0.04)'; e.currentTarget.style.borderColor = 'rgba(245,197,24,0.35)' }}>
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M4 14v5a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-5"/><path d="M7 9l5-5 5 5"/><path d="M12 4v12"/></svg>
                  Subir archivo
          ...
          {scanning && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: 12, borderRadius: 10, background: 'rgba(245,197,24,0.08)', border: '1px solid rgba(245,197,24,0.3)', marginBottom: 16 }}>
              <div style={{ width: 18, height: 18, borderRadius: '50%', border: '2px solid rgba(245,197,24,0.2)', borderTopColor: '#F5C518', animation: 'spin .7s linear infinite' }} />
              <span style={{ fontSize: 13, color: '#d8c98a' }}>Analizando documento…</span>
            </div>
          )}
